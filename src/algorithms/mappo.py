"""Basic MAPPO-style baseline with decentralized actors and centralized critic."""
...
class MAPPOTrainer:
    """MAPPO-style trainer using a centralized critic input."""
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.critic = ValueCritic(critic_obs_dim, hidden_sizes).to(self.device)
        ...
        self.method = str(algo.get("name", "mappo"))
        path_values = {"seed": self.seed, "method": self.method}
        self.log_path = str(logging.get("log_path", "logs/mappo_seed_{seed}.csv")).format(
            **path_values
        )
    ...
    def collect_rollouts(self, start_episode: int) -> tuple[RolloutBatch, list[dict[str, float | int]]]:
        ...
        critic_obs_by_agent: dict[str, list[np.ndarray]] = {
        ...
        for episode_offset in range(self.episodes_per_update):
            ...
            while not done and self.env.env.agents:
                global_obs = self.env.global_observation(observations)
                global_obs_tensor = tensor(global_obs[None, :], self.device)
                action_dict: dict[str, np.ndarray] = {}
                value = 0.0

                with torch.no_grad():
                    value = float(self.critic(global_obs_tensor).item())
                ...
                for agent in self.env.env.agents:
                    ...
                    critic_obs_by_agent[agent].append(global_obs)
                    actions_by_agent[agent].append(action)
                    log_probs_by_agent[agent].append(float(log_prob_tensor.item()))
                ...
                for agent in action_dict:
                    rewards_by_agent[agent].append(float(rewards.get(agent, 0.0)))
        ...
        for agent in self.env.possible_agents:
            ...
            flat_critic_obs.extend(critic_obs_by_agent[agent])
    ...
    def update(self, batch: RolloutBatch) -> dict[str, float]:
        ...
        critic_obs = tensor(batch.critic_obs, self.device)
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, batch_size, self.minibatch_size):
                ...
                new_log_probs, entropy = self.actor.log_prob_entropy(obs[mb_idx], actions[mb_idx])
                values = self.critic(critic_obs[mb_idx])

                ratio = torch.exp(new_log_probs - old_log_probs[mb_idx])
                unclipped = ratio * advantages[mb_idx]
                clipped = torch.clamp(ratio, 1.0 - self.clip_coef, 1.0 + self.clip_coef) * advantages[mb_idx]
    ...
    def train(self) -> list[dict[str, float | int]]:
        ...
        for start_episode in range(0, self.total_episodes, self.episodes_per_update):
            ...
            if start_episode % max(self.log_interval, 1) == 0:
                print(
                    f"[MAPPO] episode={start_episode + len(episode_rows):04d} "
